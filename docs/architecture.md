...
Server logging uses a custom console-backed logger (`src/lib/logger/workerSafeLogger.ts`). It implements the Pino subset Payload uses, respects `NEXT_PUBLIC_LOG_LEVEL`, and normalizes `Error` objects into plain serializable ones. Error tracking uses Sentry (`@sentry/nextjs`): `src/instrumentation.ts` / `src/instrumentation-client.ts` initialize it, `next.config.mjs` wraps with `withSentryConfig`, and `src/app/global-error.tsx` reports React errors. Source maps upload when `SENTRY_AUTH_TOKEN` is set at build time. **The `environment` tag comes from the Railway environment name, never `NODE_ENV`** — Railway builds a preview with `NODE_ENV=production`, so reading it tagged every preview error `production` and made preview noise indistinguishable from a real incident (#733). `deploymentEnvironment()` (`@/lib/env/deploymentEnvironment`) is the one definition, shared with the storage isolation guard's `railwayEnvironmentName()`, and falls back to `NODE_ENV` off-Railway so a local run still reports `development`. ⚠ **The two client-side call sites still read `NODE_ENV`** (`src/instrumentation-client.ts`, `src/components/ErrorBoundary.tsx`) — a browser bundle reads no `process.env` at runtime, so the name has to be inlined at build time under a `NEXT_PUBLIC_*` key. That needs **no new dashboard variable**: Railway exposes every service variable to the build (`next.config.mjs`'s `headers()` already relies on this, and `scripts/postinstall.cjs` reads `RAILWAY_*` during build), and each environment builds separately, so a `next.config.mjs` `env` entry mapping `RAILWAY_ENVIRONMENT_NAME` inlines that deployment's own name. #737 tracks it. `SENTRY_TRACES_SAMPLE_RATE` (default `0.1`) enables a DB-span breakdown on every admin request. `meditations.recomputeNodeWeights` and `frames.cascadeNodeChange` also carry manual spans (#529).
