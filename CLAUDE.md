...
- **Lessons** (`src/collections/content/Lessons.ts`) - Meditation lessons with audio upload, panels array for content sections, color theme, order within unit, and optional relationships to meditations and articles
- **LessonUnits** (`src/collections/content/LessonUnits.ts`) - Organizational units for grouping meditation lessons with color theme and virtual lesson count field
...
### Lessons & LessonUnits Architecture

The system provides meditation lesson organization through two related collections:

#### LessonUnits Collection
...
- **Type**: Upload collection (audio files)
- **Fields**:
  - `title` (text, required) - Lesson name
  - `thumbnail` (relationship to Media, required) - Visual thumbnail
  - `color` (text with color picker, required) - Theme color for the lesson
  - `unit` (relationship to LessonUnits, required) - Parent unit for organization
  - `order` (number, required) - Order within the unit (must be unique per unit)
  - `meditation` (relationship to Meditations, optional) - Related guided meditation
  - `article` (relationship to Pages, optional) - Deep dive article for additional content
  - `panels` (array, required, min 1) - Content panels, each containing:
    - `title` (text, required) - Panel heading
    - `text` (text, required) - Panel content
    - `image` (relationship to Media, required) - Panel visual
...
A comprehensive import script that migrates "Path Step" data from Storyblok CMS into Payload's **Lessons** and **LessonUnits** collections.
...
**Data Transformation**:
- **Storyblok Path Steps** → **Lessons Collection**: Title, panels (text/video), meditation references, intro audio/subtitles, deep dive articles
- **Storyblok Units** → **LessonUnits Collection**: Unit grouping with color themes and step icons
...
- Target collections: `lessons`, `lesson-units`, `file-attachments`, `external-videos`, `media`
