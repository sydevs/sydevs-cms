...
- **Articles** (`src/collections/content/Articles.ts`) - Rich text content with embedded blocks using Lexical editor, categories, tags, auto-generated slugs, and publish scheduling
...
### Articles Collection & Rich Text Architecture

The **Articles Collection** uses Payload's Lexical rich text editor with embedded blocks, enabling editors to create flexible article content with inline formatting and block components.

#### Articles Collection Structure
- **Location**: `src/collections/content/Articles.ts`
- **Core Fields**:
  - `title` (text, required, localized) - Article title
...
#### Testing Coverage
- **Integration Tests**: `tests/int/articles.int.spec.ts` with comprehensive test cases
...
- **Features**: All basic formatting plus:
  ...
  - Blockquote
  - Headings (H1, H2)
  - Relationship feature for linking to meditations, music, articles, and forms
  - Blocks feature for embedding custom block components
- **Usage**: Article content and other rich content areas
...
- **Implementation**: Articles collection uses `SlugField('title')` for auto-generating slugs from titles
