...
| `*/translations` publish + merge | Per-locale `_status` on all three translations globals, a per-locale publish (and that publishing one locale does not open the gate for another), the JSON column refusing an unknown key or a non-string value, and the API-client English merge — including a key nested inside a sub-group | `translations-globals`, `available-locales` + unit: `client-english-fallback.spec.ts` |
