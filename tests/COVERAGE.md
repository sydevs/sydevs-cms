...
| `wemeditate-web/translations` | The twelve #707 tabs in reading order, one leaf group per `<tab>.<sub-group>` pair, `SUBGROUP_PRESENTATION` rendering `a11y` as "Accessibility" and starting it collapsed and never reaching a top-level tab, plural keys expanded for storage and collapsed for the admin, a `strict` limit surviving that expansion | `translations-globals`, `translations-field` |
| `wemeditate-web/translations` seed | The English seed file against its schema (every key covered, none undeclared, none blank, every `%{placeholder}` named in its description, every `strict` value inside its limit, one `article.general.tag_*` key per `PAGE_TAGS` entry), then the real importer against a database: English written and published, no other locale published, `wm-app` left unpublished, idempotent, and `wm-web-config.availableLocales` accepting `en` but still naming French | `wm-web-translations-seed` + unit: `wm-web-translations-seed.spec.ts` |
